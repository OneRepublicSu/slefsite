...
iptFile.addEventListener('change', function (e) {
    ...
    axios.post('/api/upload/avatar', fd).then(({ data: res }) => {
        ...
        document.querySelector('.picavatar').src = 'http://39.101.141.186:8000/' + res.url
        picurl.value = 'http://39.101.141.186:8000/' + res.url
    })
})
...
function renderData(){
    axios.get('http://39.101.141.186:8000/api/getcomment').then(({data:res}) =>{
        if(res.code != 200){
        ...
        const arr = newArr.map(item =>{
            if(item.picurl == ''){
                item.picurl = "http://39.101.141.186:8000/uploads/01.jpg"
            }
            return `
